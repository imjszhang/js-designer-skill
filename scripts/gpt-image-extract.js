...
const { extractStyle } = require('../lib/api');
...
function parseExtractArgs(argv) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    else if (arg === '--role' && args[i + 1]) options.roles.push(args[++i]);
...
function printUsage() {
  console.log('\njs-designer-skill - 从参考图抽取可复用视觉风格系统');
  console.log('='.repeat(60));
  console.log('\n使用方法:');
  console.log('  node cli/index.js extract --image <path> [--role cover] [--image <path> --role inline] --brief "…"\n');
  console.log('输入选项:');
  console.log('  --image <file>              本地参考图，可重复（最多 10 张）');
  console.log('  --role <name>               与 --image 按出现顺序配对（cover|inline|table|other）');
...
async function main() {
  ...
  const result = await extractStyle(runtime, options);
...
module.exports = { main, parseExtractArgs };

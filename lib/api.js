...
const GptImageStyleExtractor = require('../scripts/gptImageStyleExtractor');
...
async function extractStyle(runtimeConfig, params = {}) {
  const normalized = normalizeReviewParams(runtimeConfig, params, {
    defaultModel: runtimeConfig.defaultExtractModel,
    enabled: runtimeConfig.extractEnabled,
  });

  if (normalized.enabled === false) {
    throw new Error(
      'extract 功能已禁用。请设置 GPT_IMAGE_EXTRACT_ENABLED=true，或在运行时配置中传入 extractEnabled: true。',
    );
  }

  const extractor = new GptImageStyleExtractor(normalized);

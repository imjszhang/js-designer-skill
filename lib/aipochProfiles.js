...
const PROFILE_DEFINITIONS = Object.freeze({
  ...
  investor: Object.freeze({
    source: referencePaths.aipochInvestorDeck.design,
    marker: 'AIPOCH INVESTOR DECK STYLE LOCK',
    requiresOptIn: true,
  }),
});
...
const PRESETS = Object.freeze({
  ...
  'investor-deck': Object.freeze(['core', 'investor']),
});
...
const INVESTOR_FORBIDDEN = /#141519/i;
...
function guardPreset(presetName, prompt) {
  const containsCoreForbidden =
    presetName !== 'website-replica' && CORE_FORBIDDEN.test(prompt);
  const containsInvestorForbidden =
    presetName === 'investor-deck' && INVESTOR_FORBIDDEN.test(prompt);
  if (containsCoreForbidden || containsInvestorForbidden) {
    throw new Error(

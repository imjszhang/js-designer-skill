...
function loadStyleLock(filePath) {
  if (!fs.existsSync(filePath)) {
    throw new Error(`AIPOCH DESIGN.md not found: ${filePath}`);
  }

  const text = fs.readFileSync(filePath, 'utf8');
  const match = text.match(/```text\r?\n(AIPOCH STYLE LOCK[\s\S]*?)```/);
  if (!match) {
    throw new Error(`AIPOCH STYLE LOCK fenced block not found in ${filePath}`);
  }
...
function withStyleLock(taskPrompt) {
  const styleLock = loadStyleLock(designSystemPath);
  return `${styleLock}

TASK
Compose inside a centered 16:9 safe frame. If the output canvas is 1536x1024, leave cropable extra height rather than stretching. Render only the visible Chinese copy named below.
